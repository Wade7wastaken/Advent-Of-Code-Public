...
fn part1(input: &str) -> u32 {
    let mut grid = Grid::new_transpose(vec![vec![false; 1000]; 1000]).unwrap();
...
fn part2(input: &str) -> u32 {
    let mut grid = Grid::new_transpose(vec![vec![0u8; 1000]; 1000]).unwrap();
    for (start, end, operation) in input.lines().map(parse_command) {
        for y in start.y..=end.y {
            for x in start.x..=end.x {
                match operation {
                    Operation::On => {
                        grid.update((x, y), |c| c.saturating_add(1));
                    }
                    Operation::Off => {
                        grid.update((x, y), |c| c.saturating_sub(1));
                    }
                    Operation::Toggle => {
                        grid.update((x, y), |c| c.saturating_add(2));
                    }
                }
            }
        }
