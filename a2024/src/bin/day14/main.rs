...
fn safety_factor(robots: Vec<Robot>) -> u32 {
    ...
    for robot in robots {
        if robot.position.within((cw + 1, 0), (WIDTH, ch)) {
            q1 += 1;
        } else if robot.position.within((0, 0), (cw, ch)) {
            q2 += 1;
        } else if robot.position.within((0, ch + 1), (cw, HEIGHT)) {
            q3 += 1;
        } else if robot.position.within((cw + 1, ch + 1), (WIDTH, HEIGHT)) {
            q4 += 1;
        }
    }
    q1 * q2 * q3 * q4
